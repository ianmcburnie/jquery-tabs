...
$tabs.each(function onEachTab(idx,el){var $tab=$(el);var tabId=$tabsWidget.attr('id')+'-tab-'+idx;var panelId=$tabsWidget.attr('id')+'-panel-'+idx;$tab.attr('id',tabId).attr('aria-controls',panelId);$panels.eq(idx).attr('id',panelId).attr('aria-labelledby',tabId);});$tablist.rovingTabindex($tabs);$tablist.on('rovingTabindexChange',function(e,selectedTab){var $selectedTab=$(selectedTab);var $activeTab=$tablist.find('[aria-selected=true]');var $activePanel=$panelcontainer.find('[aria-labelledby={0}]'.replace('{0}',$activeTab.attr('id')));var $selectedPanel=$panelcontainer.find('[aria-labelledby={0}]'.replace('{0}',$selectedTab.attr('id')));if($selectedTab[0]!==$activeTab[0]){$activePanel.attr('aria-hidden','true');$selectedPanel.attr('aria-hidden','false');$selectedTab.attr('aria-selected','true');setTimeout(function(){$activeTab.attr('aria-selected','false');$tabsWidget.trigger('tabsSelect');},0);}});$('.tabs [role=tab]').preventDocumentScrollKeys();$tabsWidget.addClass('tabs--js');});};}(jQuery));